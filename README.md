...
Arguments use terse shell-style tokens: `key=value`, `key:value`, double-dash booleans like `--confirm`, `--wake`, and `--no-cache`, explicit negated booleans like `--no-wake` and `--no-confirm`, booleans like `confirm=true`, comma-separated lists like `endpoints=charge_state,drive_state`, and one bare positional vehicle identifier for most vehicle-targeting commands.
