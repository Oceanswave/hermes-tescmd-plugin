...
Arguments use terse shell-style tokens: `key=value`, `key:value`, separated double-dash values like `--percent 80` and `--driver-temp 70`, double-dash booleans like `--confirm`, `--wake`, and `--no-cache`, explicit negated booleans like `--no-wake` and `--no-confirm`, booleans like `confirm=true` or `--confirm false`, comma-separated lists like `endpoints=charge_state,drive_state` or `--place-ids id1,id2`, and one bare positional vehicle identifier for most vehicle-targeting commands.
