...
from google.protobuf import descriptor_pb2, message  # type: ignore[import-untyped]  # noqa: F401
...
@dataclass
class HMACSignatureData:
    """Plain HMAC signature data used for session_info_tag."""

    tag: bytes = b""

    def serialize(self) -> bytes:
        if self.tag:
            return _encode_length_delimited(_TAG_HMAC_EPOCH, self.tag)
        return b""

    @staticmethod
    def parse(data: bytes) -> HMACSignatureData:
        result = HMACSignatureData()
        pos = 0
        while pos < len(data):
            fn, _wt, val, pos = _decode_field(data, pos)
            # HMAC_Signature_Data has field 1 named `tag`. This differs from
            # HMAC_Personalized_Signature_Data where field 1 is `epoch`, so a
            # session_info_tag must not be parsed as personalized data.
            if fn == _TAG_HMAC_EPOCH and isinstance(val, bytes):
                result.tag = val
        return result
...
class SignatureData:
    ...
    session_info_tag: HMACSignatureData | None = None
    ...
    def parse(data: bytes) -> SignatureData:
        ...
        while pos < len(data):
            ...
            if fn == _TAG_SD_HMAC_TAG and isinstance(val, bytes):
                ...
                result.session_info_tag = HMACSignatureData.parse(val)
        return result
...
class RoutableMessage:
    ...
    def operation_status(self) -> OperationStatus:
        """Convenience: extract operation status from message_status."""
        if self.message_status is not None:
            return self.message_status.operation_status
        return OperationStatus.OPERATIONSTATUS_OK
