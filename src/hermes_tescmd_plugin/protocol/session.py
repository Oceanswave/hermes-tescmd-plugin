...
from hermes_tescmd_plugin.errors import (
    KeyNotEnrolledError,
    SessionError,
    TeslaAPIError,
    VehicleAsleepError,
...
from hermes_tescmd_plugin.protocol.signer import (
...
if TYPE_CHECKING:
    ...
    from hermes_tescmd_plugin.client import TeslaFleetClient
...
class Session:
    ...
    def next_counter(self) -> int:
        ...
        Counter allocation is protected because Hermes may invoke tools from
        multiple threads while process-global SessionManager instances are reused.
        """
...
class SessionManager:
    ...
    async def _send_handshake(self, vin: str, domain: Domain) -> RoutableMessage:
        ...
        if not response_b64:
            logger.debug(
                "Empty handshake response for %s (%s)",
                _redact_vin(vin),
                domain.name,
            )
        ...
        logger.debug(
            "Handshake response for %s (%s): %d bytes",
            _redact_vin(vin),
            domain.name,
            len(response_bytes),
        )
    ...
    def _build_session(self, vin: str, domain: Domain, response_msg: RoutableMessage) -> Session:
        ...
        # Verify session info HMAC tag. A missing/empty tag is a hard protocol
        # failure; accepting unsigned SessionInfo would weaken the handshake.
        sig = response_msg.signature_data
        si_tag = sig.session_info_tag if sig else None
        if not (si_tag and si_tag.tag):
            raise SessionError(
                f"Missing session info HMAC tag for {_redact_vin(vin)} ({domain.name})."
            )
        if not verify_session_info_tag(
            session_info_key,
            response_msg.session_info,
            si_tag.tag,
            verifier_id=vin.encode(),
            challenge=response_msg.request_uuid,
        ):
            raise SessionError(
                f"Session info HMAC verification failed for {_redact_vin(vin)} ({domain.name}). "
