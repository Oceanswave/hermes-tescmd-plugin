...
def verify_session_info_tag(
    ...
    *,
    ...
    """
    # Tesla's Go implementation computes SessionInfoHMAC over authenticated
    # metadata plus the encoded SessionInfo:
    #   TAG_SIGNATURE_TYPE = SIGNATURE_TYPE_HMAC (6)
    #   TAG_PERSONALIZATION = verifier ID (Fleet VIN)
    #   TAG_CHALLENGE = request UUID copied into response.request_uuid
    # then a bare TAG_END byte (0xFF), then encoded SessionInfo bytes.
    metadata = b"".join(
        (
            encode_tlv(TAG_SIGNATURE_TYPE, bytes([_SIGNATURE_TYPE_HMAC])),
            encode_tlv(TAG_PERSONALIZATION, verifier_id),
