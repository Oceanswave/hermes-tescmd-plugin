...
The plugin uses hand-written protobuf payload builders in ``payloads.py`` for
the command bodies required by Tesla's signed-command endpoint. Keep new
commands registered here and backed by explicit payload-builder coverage rather
than silently falling back to opaque JSON bytes.
"""
...
from hermes_tescmd_plugin.protocol.protobuf.messages import Domain
...
def requires_signing(command: str) -> bool:
    ...
    Unknown commands fail closed: callers must add a registry entry rather than
    silently falling back to legacy REST command transport.
    """
    spec = COMMAND_REGISTRY.get(command)
    if spec is None:
        raise ValueError(f"Unknown Tesla vehicle command registry entry: {command}")
