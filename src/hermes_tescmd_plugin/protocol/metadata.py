...
Tags (from Tesla's vehicle-command ``signatures.proto`` Tag enum):
  ...
  - 0x04: expires_at (4 bytes, big-endian uint32 — vehicle epoch-relative seconds)
...
def encode_metadata(
    *,
    ...
    expires_at:
        Command expiration time (vehicle epoch-relative seconds).
    counter:
    ...
    """
    expires_at = _require_uint32("expires_at", expires_at)
    counter = _require_uint32("counter", counter)
