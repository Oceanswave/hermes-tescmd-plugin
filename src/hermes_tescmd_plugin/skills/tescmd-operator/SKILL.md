...
1. Do Tesla Developer app creation/callback/scope setup outside Hermes, following `docs/ONBOARDING.md` and the README. Tesla app callback URLs must be public HTTPS URLs; configure `domain` for the default `https://<domain>/callback` or set `oauth_redirect_uri` explicitly. Non-secret fields (`client_id`, `region`, `domain`, `oauth_redirect_uri`, `default_vin`, `scopes`, `redirect_port`) can be edited in the Hermes plugin config store at `plugins.entries.hermes-tescmd-plugin.config.profiles.<profile>` when the host exposes plugin-provided config. Legacy `HERMES_HOME/plugins/hermes-tescmd-plugin/config.json` still works and migrates non-secret values into the Hermes store when available. Keep secrets (`client_secret`, vehicle-command key paths, Google Maps API keys, tokens, PINs) out of dashboard-editable config; there is intentionally no `tescmd_setup` tool.
...
- The stateful setup wizard, `tescmd_setup` config mutation tool, Tailscale hosting automation, and GitHub Pages hosting automation are intentionally absent. Keep Tesla Developer app setup, domain hosting, and virtual-key enrollment as explicit operator-managed steps documented in `docs/ONBOARDING.md` and the README. Only non-secret app defaults are dashboard/config-store editable; secret fields stay in auth/plugin-owned state.
