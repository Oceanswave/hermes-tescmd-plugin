...
def _normalize_domain(value: Any, *, allow_local: bool = False) -> str | None:
    ...
    if ascii_host in {"localhost", "localhost.localdomain"}:
        raise client.TeslaAPIError(
            "domain must be public HTTPS hostname, not localhost."
        )
    try:
...
def _safe_export_path(path_text: str, *, profile: str) -> Path:
    ...
    if root.resolve() not in candidate.parents and candidate != root.resolve():
        raise client.TeslaAPIError(
            f"output_path must stay under the plugin export directory: {root}"
        )
    if candidate.exists():
...
def _resolve_vin(args: dict[str, Any], profile: str) -> str:
    ...
    if not vin:
        raise client.TeslaAPIError(
            "No vehicle identifier was provided and no default vehicle identifier is configured for this profile."
        )
...
def _resolve_full_vin(
    args: dict[str, Any], fleet_client: client.TeslaFleetClient, *, operation: str
) -> str:
    ...
    for vehicle in vehicles:
        ...
        identifiers = {
            str(vehicle.get(key))
            for key in ("id_s", "vehicle_id", "id", "vin")
        ...
        if (
            ...
            and client.is_tesla_vin(candidate)
        ):
            return candidate
    return _require_full_vin(value, operation=operation)


def _resolve_full_vins(
    args: dict[str, Any], fleet_client: client.TeslaFleetClient, *, operation: str
) -> list[str]:
    ...
    if requested is None:
        ...
        raise client.TeslaAPIError(
            "vins must be an array of full VIN strings or a comma-separated string."
        )
    vins = [
        _require_full_vin(str(value), operation=operation) for value in values if value
    ]
    if not vins:
        raise client.TeslaAPIError(
            f"{operation} requires at least one full 17-character VIN."
        )
    return vins
...
def _fleet_client(args: dict[str, Any]) -> client.TeslaFleetClient:
    return client.TeslaFleetClient(
        profile=_profile(args), region_override=args.get("region")
    )
...
def _result_from_command(
    command_name: str, vin: str, response: Any, *, region: str
) -> dict[str, Any]:
    ok = True
...
def _result_payload(
    *,
    fleet_client: client.TeslaFleetClient,
    result: Any,
    result_key: str,
    vin: str | None = None,
...
def _key_presence(profile: str) -> tuple[Path | None, Path | None, bool]:
    ...
    present = bool(
        private_path and public_path and private_path.exists() and public_path.exists()
    )
    ...
    *,
    ...
    return {
        ...
        "enrollment_ready": bool(
            key_validation["matches_local_key"] and cfg.client_secret
        ),
    }
...
def _bootstrap_next_steps(
    *, cfg: config.PluginConfig, bootstrap: dict[str, Any], redirect_uri: str | None
    ...
    if (
        bootstrap["key_present"]
        and cfg.domain
        and not bootstrap["public_key_accessible"]
    ):
        return (
            "key_validate",
            [
    ...
    if (
        bootstrap["key_present"]
        and cfg.domain
        and not bootstrap["public_key_matches_local_key"]
    ):
        return (
            "key_deploy",
            [
...
def _bootstrap_payload(
    *,
    ...
    next_action, next_steps = _bootstrap_next_steps(
        cfg=cfg, bootstrap=bootstrap, redirect_uri=redirect_uri
    )
    _, public_path, key_present = _key_presence(profile)
    expected_public_key_url = (
        f"https://{cfg.domain}/{WELL_KNOWN_PATH}"
        if cfg.domain and key_present
        else None
    )
    enrollment_url = (
...
def handle_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        or [scope for scope in cfg.scopes if scope not in config.PARTNER_ONLY_SCOPES],
        "partner_only_scopes": [
        ...
        "audit": {
            ...
            "hermes_agent_log_path": str(
                config.get_hermes_home() / "logs" / "agent.log"
            ),
...
def _mapped_payload(spec: Any, args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if payload_mode == "managed_location":
        ...
        if lat is None or lon is None:
            raise client.TeslaAPIError(
                "Both lat and lon are required for managed charger location."
            )
    ...
    if payload_mode == "navigation_place_ids":
        ...
        if not isinstance(raw_place_ids, list) or not raw_place_ids:
            raise client.TeslaAPIError(
                "place_ids must be a non-empty array of Google Maps Place IDs."
            )
        encoded: list[str] = []
        for item in raw_place_ids:
            ...
            if not place_id or any(ch in place_id for ch in ",\x00"):
                raise client.TeslaAPIError(
                    "Google Maps Place IDs must be non-empty and must not contain commas or NUL bytes."
                )
...
def _require_confirm(args: dict[str, Any], action: str) -> None:
    if not args.get("confirm"):
        raise client.TeslaAPIError(
            f"confirm=true is required before running high-risk action: {action}."
        )
...
def _handle_vehicle_command(spec: Any, args: dict[str, Any]) -> dict[str, Any]:
    ...
    response = fleet_client.vehicle_command(
        vin, spec.command_name, _mapped_payload(spec, args)
    )
    return _result_from_command(
        spec.command_name, vin, response, region=fleet_client.region
    )
...
def _public_key_paths(profile: str) -> tuple[Path | None, Path | None]:
    cfg = config.load_config(profile)
    private_path = (
        Path(cfg.vehicle_command_key_private_path)
        if cfg.vehicle_command_key_private_path
        else None
    )
    public_path = (
        Path(cfg.vehicle_command_key_public_path)
        if cfg.vehicle_command_key_public_path
        else None
    )
...
def _hosted_key_validation(*, profile: str, cfg: config.PluginConfig) -> dict[str, Any]:
    ...
    local_fingerprint = (
        _public_key_fingerprint(public_path)
        if public_path and public_path.exists()
        else None
    )
    if not cfg.domain:
    ...
    remote_fingerprint = None
    try:
        response = client.httpx.get(
            f"https://{cfg.domain}/{WELL_KNOWN_PATH}", follow_redirects=True, timeout=10
        )
        remote_fingerprint = (
            _pem_fingerprint(response.text) if response.status_code == 200 else None
        )
    ...
    return {
        ...
        "matches_local_key": bool(
            accessible
            and local_fingerprint
            and remote_fingerprint
            and local_fingerprint == remote_fingerprint
        ),
    }
...
def handle_auth_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    auth_state = config.load_auth_state(profile)
    pending = config.load_pending_auth(profile)
    payload["bootstrap"] = _bootstrap_status(
        profile=profile, cfg=cfg, auth_state=auth_state, pending=pending
    )
    return payload
...
def handle_auth_import(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(auth_payload, (dict, str)):
        raise client.TeslaAPIError(
            "auth must be an exported auth object or JSON string."
        )
...
def handle_key_generate(args: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = auth.generate_vehicle_command_keypair(
        profile, domain=config.load_config(profile).domain, force=force
    )
...
def handle_key_show(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not present:
        return {
            ...
            "profile": profile,
            "status": "not_found",
            "key_dir": str(config.get_plugin_home() / "keys" / profile),
        }
    ...
    return {
        ...
        "public_key_path": str(public_path),
        "fingerprint": _public_key_fingerprint(public_path),
        "expected_public_key_url": f"https://{cfg.domain}/{WELL_KNOWN_PATH}"
        if cfg.domain
        else None,
...
def handle_key_validate(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not cfg.domain:
        raise client.TeslaAPIError(
            "No domain configured. Edit the plugin config file with a domain first."
        )
...
def handle_key_enroll(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (key_status["accessible"] and key_status["matches_local_key"]):
        raise client.TeslaAPIError(
            "Hosted public key must be reachable and match the local key before enrollment. Run tescmd_key_deploy and tescmd_key_validate first."
        )
    return {
        "ok": True,
        "profile": profile,
        ...
        "methods": [
            {
                ...
                "steps": "Controls > Locks > remove the virtual key > scan key card",
                "speed": "immediate",
            },
            {
                "name": "tesla_app",
            ...
            },
            {
                ...
                "steps": "accounts.tesla.com > Security > Third Party Apps > Manage > Remove",
                "speed": "up to 2 hours",
            },
        ],
    }
...
def handle_key_deploy(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not public_path or not public_path.exists():
        raise client.TeslaAPIError(
            "No public key found. Run tescmd_key_generate first."
        )

    requested_method = args.get("method") or "local"
    if requested_method != "local":
        raise client.TeslaAPIError(
            "Manual HTTPS hosting only: use method='local', then serve the generated .well-known public-key file from your own domain. Tailscale, GitHub Pages, and MCP/server hosting integrations are intentionally not included in this plugin."
        )
    ...
    (deploy_root / "index.html").write_text(
        "<html><body><p>Tesla Fleet API public key host.</p></body></html>\n"
    )
    return {
        "ok": True,
        "profile": profile,
...
def handle_vehicle_get(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        ...
        vin=vin,
        result=fleet_client.vehicle(vin),
        result_key="vehicle",
    )


def _cached_vehicle_status(
    fleet_client: client.TeslaFleetClient,
    vin: str,
    *,
    endpoints: Any,
    ...
    vehicle_cache_id = hashlib.sha256(
        f"{fleet_client.profile}:{fleet_client.region}:{vin}".encode("utf-8")
    ).hexdigest()
...
def handle_vehicle_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    data, cache_meta = _cached_vehicle_status(
        fleet_client,
        vin,
        endpoints=args.get("endpoints"),
        no_cache=bool(args.get("no_cache", False) or args.get("wake")),
    )
    return {
        "ok": True,
        "profile": fleet_client.profile,
        ...
        "cache_bypassed": cache_meta["bypassed"],
        "cache": cache_meta,
    }
...
def _handle_vehicle_data_section(
    args: dict[str, Any],
    endpoint: str,
    *,
...
def handle_vehicle_location(args: dict[str, Any]) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "location_data", result_key="location", aliases=("drive_state",)
    )


def handle_vehicle_drive_status(args: dict[str, Any]) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "drive_state", result_key="drive_state", aliases=("location_data",)
    )
...
def handle_vehicle_config_status(args: dict[str, Any]) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "vehicle_config", result_key="vehicle_config"
    )
...
def handle_vehicle_charge_schedule_status(args: dict[str, Any]) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "charge_schedule_data", result_key="charge_schedule"
    )


def handle_vehicle_preconditioning_schedule_status(
    args: dict[str, Any],
) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "preconditioning_schedule_data", result_key="preconditioning_schedule"
    )
...
def handle_vehicle_wake(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.wake_vehicle(vin),
        result_key="response",
    )
...
def handle_vehicle_mobile_access(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.mobile_enabled(vin),
        result_key="mobile_access_enabled",
    )
...
def handle_vehicle_nearby_chargers(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.nearby_charging_sites(vin),
        result_key="sites",
    )
...
def handle_vehicle_alerts(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.recent_alerts(vin),
        result_key="alerts",
    )
...
def handle_vehicle_drivers(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.drivers(vin),
        result_key="drivers",
    )
...
def handle_vehicle_release_notes(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.release_notes(vin),
        result_key="release_notes",
    )
...
def handle_vehicle_service(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.service_data(vin),
        result_key="service",
    )
...
def handle_vehicle_subscriptions(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.subscriptions(vin),
...
def handle_vehicle_upgrades(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.upgrades(vin),
        result_key="upgrades",
    )
...
def handle_vehicle_options(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.options(vin),
        result_key="options",
    )
...
def handle_vehicle_warranty(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.warranty_details(vin=vin),
        result_key="warranty",
    )
...
def handle_vehicle_pricing(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.vehicle_pricing(request_payload),
        result_key="pricing",
    )
...
def handle_vehicle_enterprise_roles(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.enterprise_roles(vin),
        result_key="roles",
    )
...
def handle_vehicle_enterprise_payer(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.enterprise_payer(vin, payer),
        result_key="response",
    )
...
def handle_vehicle_fleet_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.fleet_status(vins=vins),
        result_key="fleet_status",
    )
...
def handle_vehicle_telemetry_config(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.fleet_telemetry_config(vin),
        result_key="telemetry_config",
    )
...
def handle_vehicle_telemetry_create(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.fleet_telemetry_config_create(
...
def handle_vehicle_telemetry_create_jws(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.fleet_telemetry_config_jws(token),
        result_key="response",
    )
...
def handle_vehicle_telemetry_delete(args: dict[str, Any]) -> dict[str, Any]:
    if not args.get("confirm"):
        raise client.TeslaAPIError(
            "confirm=true is required to delete fleet telemetry configuration."
        )
    fleet_client = _fleet_client(args)
    vin = _resolve_vin(args, fleet_client.profile)
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.fleet_telemetry_config_delete(vin),
        result_key="response",
    )
...
def handle_vehicle_telemetry_errors(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.fleet_telemetry_errors(vin),
        result_key="errors",
    )
...
def handle_climate_status(args: dict[str, Any]) -> dict[str, Any]:
    return _handle_vehicle_data_section(
        args, "climate_state", result_key="climate_state"
    )
...
def handle_software_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = handle_vehicle_status(args)
    vehicle_state = (
        payload["data"].get("vehicle_state")
...
def handle_user_me(args: dict[str, Any]) -> dict[str, Any]:
    fleet_client = _fleet_client(args)
    return _result_payload(
        fleet_client=fleet_client, result=fleet_client.user_me(), result_key="user"
    )
...
def handle_user_region(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.user_region(),
        result_key="region_info",
    )
...
def handle_user_orders(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.user_orders(),
        result_key="orders",
    )
...
def handle_user_features(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.user_feature_config(),
        result_key="features",
    )
...
def handle_billing_history(args: dict[str, Any]) -> dict[str, Any]:
    fleet_client = _fleet_client(args)
    result = fleet_client.charging_history(
        vin=args.get("vin_filter") or args.get("vin"),
    ...
    return _result_payload(
        fleet_client=fleet_client, result=result, result_key="history"
    )


def handle_billing_sessions(args: dict[str, Any]) -> dict[str, Any]:
    fleet_client = _fleet_client(args)
    result = fleet_client.charging_sessions(
        vin=args.get("vin_filter") or args.get("vin"),
    ...
    return _result_payload(
        fleet_client=fleet_client, result=result, result_key="sessions"
    )
...
def handle_billing_invoice(args: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = _result_payload(
        fleet_client=fleet_client, result=result, result_key="invoice"
    )
    if args.get("output_path") and isinstance(result, str):
        _require_confirm(args, "billing_invoice_output")
        target = _safe_export_path(
            str(args["output_path"]), profile=fleet_client.profile
        )
...
def handle_energy_list(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_products(),
        result_key="products",
    )
...
def handle_energy_live(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_live_status(site_id),
        result_key="live_status",
...
def handle_energy_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_site_info(site_id),
        result_key="site",
        extra={"site_id": site_id},
    )
...
def handle_energy_backup(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_backup(
            site_id, _validate_percent("percent", args["percent"])
        ),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_mode(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_operation_mode(site_id, str(args["mode"])),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_storm(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_storm_mode(site_id, bool(args["enabled"])),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_tou(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_time_of_use(site_id, settings),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_calendar(args: dict[str, Any]) -> dict[str, Any]:
    fleet_client = _fleet_client(args)
    site_id = _resolve_site_id(args)
    result = fleet_client.energy_calendar_history(
        site_id,
        kind=args.get("kind") or "energy",
...
def handle_energy_off_grid(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_off_grid_vehicle_charging_reserve(
            site_id, _validate_percent("reserve", args["reserve"])
        ),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_grid_config(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.energy_grid_import_export(site_id, config_payload),
        result_key="response",
        extra={"site_id": site_id},
    )
...
def handle_energy_telemetry(args: dict[str, Any]) -> dict[str, Any]:
    fleet_client = _fleet_client(args)
    site_id = _resolve_site_id(args)
    result = fleet_client.energy_telemetry_history(
        site_id,
        kind=args.get("kind") or "charge",
        start_date=args.get("start_date"),
        end_date=args.get("end_date"),
        time_zone=args.get("time_zone"),
    )
...
def handle_partner_public_key(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.partner_public_key(str(domain)),
...
def handle_partner_telemetry_error_vins(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.partner_fleet_telemetry_error_vins(),
        result_key="vins",
    )
...
def handle_partner_telemetry_errors(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.partner_fleet_telemetry_errors(),
        result_key="errors",
    )
...
def handle_sharing_add_driver(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.add_driver(vin, str(email)),
        result_key="driver",
    )
...
def handle_sharing_remove_driver(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.remove_driver(vin, int(share_user_id)),
        result_key="response",
    )
...
def handle_sharing_create_invite(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.create_invite(vin),
        result_key="invite",
    )
...
def handle_sharing_list_invites(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.list_invites(vin),
        result_key="invites",
    )
...
def handle_sharing_redeem_invite(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.redeem_invite(str(code)),
        result_key="invite",
    )
...
def handle_sharing_revoke_invite(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        vin=vin,
        result=fleet_client.revoke_invite(vin, str(invite_id)),
        result_key="response",
    )
...
def _validate_raw_path(path: Any) -> str:
    ...
    if not path_text.startswith("/api/"):
        raise client.TeslaAPIError(
            "Raw Fleet API paths must be relative paths starting with /api/."
        )
    if "://" in path_text or ".." in path_text or "\x00" in path_text:
        raise client.TeslaAPIError(
            "Raw Fleet API paths must not be absolute URLs, contain NUL bytes, or contain parent-directory traversal."
        )
    return path_text
...
def handle_raw_get(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.raw_get(path, params=params),
        result_key="response",
        extra={"path": path, "method": "GET"},
    )
...
def handle_raw_post(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.raw_post(path, body=body),
        result_key="response",
        extra={"path": path, "method": "POST"},
    )
...
def handle_raw_delete(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _result_payload(
        fleet_client=fleet_client,
        result=fleet_client.raw_delete(path, body=body),
        result_key="response",
        extra={"path": path, "method": "DELETE"},
    )
...
def handle_navigation_place_search(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not cfg.google_maps_api_key:
        raise client.TeslaAPIError(
            "google_maps_api_key is not configured. Add google_maps_api_key to the plugin config file, or provide Google Maps Place IDs directly to tescmd_navigation_waypoints."
        )
    ...
    try:
        ...
        raise client.TeslaAPIError(
            f"Google Places returned non-JSON response with HTTP {response.status_code}."
        ) from exc
    if response.status_code >= 400:
        ...
        raise client.TeslaAPIError(
            f"Google Places search failed with HTTP {response.status_code}: {message or payload}"
        )
    ...
    return {
        ...
        "next_args_hint": {
            "place_ids": [
                item["place_id"] for item in candidates if item.get("place_id")
            ]
        },
