...
LOCATION_KEYS = {
    "lat", "latitude", "lon", "lng", "longitude", "location", "location_data",
    "drive_state", "native_latitude", "native_longitude", "active_route_latitude",
    "active_route_longitude", "address", "formatted_address", "site_name",
    "installation_address", "home", "work",
}
TOKENISH_RE = re.compile(r"(?i)(access_token|refresh_token|client_secret|code|state|authorization|bearer)[^,}\n]*")
...
def redact_text(text: str) -> str:
    ...
    text = TOKENISH_RE.sub(lambda m: m.group(0).split(":", 1)[0] + ":[REDACTED]" if ":" in m.group(0) else "[REDACTED]", text)
    return text
...
def summarize(value: Any, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for k, v in list(value.items())[:20]:
            ...
            if lk in LOCATION_KEYS or any(s in lk for s in ("vin", "token", "secret", "authorization", "state", "code", "email", "name", "profile_image", "vault", "domain", "url", "uri", "fingerprint", "key_path")):
    ...
    if isinstance(value, list):
        return {"count": len(value), "sample": [summarize(v, depth + 1) for v in value[:2]]}
...
def call(name: str, args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "error": redact_text(str(payload.get("error", "")))[:500] if not payload.get("ok") else "",
...
def main() -> None:
    ...
    if isinstance(vehicle_raw, dict):
        for vehicle in vehicle_raw.get("vehicles", []):
            if isinstance(vehicle, dict) and str(vehicle.get("access_type", "")).upper() == "OWNER":
                owner_vehicle_id = vehicle.get("id_s") or vehicle.get("vin")
                break
    if auto_wake and owner_vehicle_id:
        for _ in range(3):
            wake_result = call("tescmd_vehicle_wake", {"vin": owner_vehicle_id, "confirm": True})
            prereq.append(wake_result)
            if wake_result["ok"] and "online" in json.dumps(wake_result.get("summary", {})).lower():
                break
            time.sleep(10)
    try:
    ...
    # correctly.
    energy_products = energy_raw.get("products", []) if isinstance(energy_raw, dict) else []
    site_id = None
    for product in energy_products:
        if isinstance(product, dict) and product.get("device_type") == "energy" and product.get("resource_type") == "battery":
    ...
    tasks.extend([
        ...
        ("tescmd_vehicle_status", {**status_read_args, "endpoints": ["charge_state", "climate_state", "closures_state", "drive_state", "gui_settings", "location_data", "vehicle_config", "vehicle_state", "charge_schedule_data", "preconditioning_schedule_data"]}),
        ...
        ("tescmd_vehicle_drivers", {"vin": owner_vehicle_id} if owner_vehicle_id else {}),
        ...
        ("tescmd_sharing_list_invites", {"vin": owner_vehicle_id} if owner_vehicle_id else {}),
        ("tescmd_cache_status", {}),
    ])
    if site_id:
        tasks.extend([
            ...
            ("tescmd_energy_calendar", {"site_id": site_id, "kind": "energy", "period": "day"}),
            ("tescmd_energy_history", {"site_id": site_id, "start_date": start, "end_date": end}),
            ("tescmd_energy_telemetry", {"site_id": site_id, "kind": "charge", "start_date": start, "end_date": end}),
        ])
    if cfg.google_maps_api_key:
        tasks.append(("tescmd_navigation_place_search", {"query": "Tesla Fremont Factory", "limit": 1}))

    results = prereq + [call(name, args) for name, args in tasks]
    ok = sum(1 for r in results if r["ok"])
    out = {"generated_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "auto_wake": auto_wake, "site_id_present": bool(site_id), "total": len(results), "ok": ok, "failed": len(results)-ok, "results": results}
