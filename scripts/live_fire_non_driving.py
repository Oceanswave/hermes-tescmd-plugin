...
def redact(obj: Any) -> Any:
    ...
    if isinstance(obj, str):
        ...
        s = re.sub(
            r"(access_token|refresh_token|client_secret|code|state)=([^&\s]+)",
            r"\1=[REDACTED]",
            s,
            flags=re.I,
        )
...
def find_target(
    ...
    vehicles = [
        ...
        and (p.get("vin") or p.get("id_s") or p.get("vehicle_id"))
    ]
    needle = target.lower()
    matches = []
    for v in vehicles:
        hay = " ".join(
            str(v.get(k, ""))
            for k in ("display_name", "vehicle_name", "name", "model", "car_type")
        ).lower()
    ...
    if chosen:
        ...
        identifier = (
            vin
            if vin and VIN_RE.fullmatch(vin)
            else chosen.get("id_s") or str(chosen.get("vehicle_id") or "")
        )
...
def main() -> int:
    ...
    if not target:
        print(
            json.dumps(
                {
                    "ok": False,
                    "error": "Set TESCMD_LIVE_TARGET to a vehicle display name or identifier before running live-fire tests.",
                }
            )
        )
        return 2
    out_path = Path(
        os.environ.get("TESCMD_LIVE_FIRE_OUT", "/tmp/tescmd-live-fire-redacted.json")
    )
    handlers = {
    ...
    report: dict[str, Any] = {
        ...
        "skipped": [],
        "notes": [
            "All side-effecting tool calls use confirm=true.",
            "Artifact is redacted before write.",
        ],
    }
    ...
    for name, args in [
        ...
        if name == "tescmd_vehicle_wake" and not report["steps"][-1]["ok"]:
            ...
            out_path.write_text(
                json.dumps(redact(report), indent=2, sort_keys=True) + "\n"
            )
            print(
                json.dumps(
                    {"ok": False, "error": report["error"], "out": str(out_path)}
                )
            )
            return 3
    ...
    for attempt in range(1, 13):
        report["steps"].append(
            wait_step(
                10 if attempt > 1 else 3, f"wait for target wake attempt {attempt}"
            )
        )
    ...
    report["steps"].append(
        call(handlers, "tescmd_vehicle_mobile_access", {"vin": identifier})
    )
    if online_info_step is None:
        report["ok"] = False
        report["error"] = (
            "target did not come online/readable after wake polling; no vehicle controls sent after preflight"
        )
        out_path.write_text(json.dumps(redact(report), indent=2, sort_keys=True) + "\n")
        print(json.dumps({"ok": False, "error": report["error"], "out": str(out_path)}))
        return 4
    ...
    report["preflight_summary"] = redact(
        {
            ...
            "latitude_present": bool(drive_state.get("latitude"))
            if isinstance(drive_state, dict)
            else False,
        }
    )
    ...
    command_sequence: list[tuple[str, dict[str, Any], str]] = [
        ...
        (
            ...
            {**side, "seat_position": 0, "level": 1},
            "driver seat heat low",
        ),
        (
            "tescmd_climate_seat_heater",
            {**side, "seat_position": 0, "level": 0},
            "driver seat heat off",
        ),
        (
    ...
    if locked is True:
        command_sequence.extend(
            [
                (
                    ...
                    side,
                    "unlock from locked state; relock next",
                ),
        ...
        report["skipped"].append(
            {"tool": "lock_unlock", "reason": "could not determine initial lock state"}
        )

    charging_state = (
        str(charge_state.get("charging_state", "")).lower()
        if isinstance(charge_state, dict)
        else ""
    )
    if charging_state in {"charging", "stopped", "complete", "no power"}:
        ...
        report["skipped"].append(
            {
                "tool": "tescmd_charge_start/stop",
                "reason": f"charging_state not safely connected: {charging_state or 'unknown'}",
            }
        )
    ...
    # Final state read.
    report["steps"].append(
        call(handlers, "tescmd_vehicle_info", {**base, "wake": True, "confirm": True})
    )
    report["completed_at"] = datetime.now(timezone.utc).isoformat()
    report["ok"] = all(
        step.get("ok", True) for step in report["steps"] if step.get("tool") != "wait"
    )
        ...
        json.dumps(
            {
                ...
                "out": str(out_path),
                "stopped_after_failure": report.get("stopped_after_failure"),
            }
        )
    )
